...
const ChipInfo = ({
  ...
  if (clickable) {
    clickableStyles = selectedList.includes(channelName) ? clickableSelectedStyles : clickableDefaultStyles;
  }
  else {
  ...
  return (
    <div>
      <ChipComponent
        ...
        className={classname}
        clickable={clickable}
        sx={{ ...commonStyles, ...clickableStyles, "& .MuiChip-label": { padding: 0 }, borderRadius: shape === "circle" ? "3rem" : "0.375rem" }}
      />
    </div>
  );
};
