...
export const InfoModal = ({open, onClose, titleText, descriptionText, text}: InfoModalProps) => {
  ...
  return (
    <Modal open={open} onClose={onClose}>
        <Box
          className='position-fixed top-0 left-0 w-full h-full bg-text-primary bg-opacity-60 flex items-center justify-center'
        >
            <Grid className='h-[372px] w-[470px] bg-text-hint rounded-md'>
                ...
                <div className='flex flex-col justify-center items-center gap-4 py-4'>
                    <Grid 
                        className='h-[140px] w-[140px] rounded-full bg-background-primaryWeak'
                    />
                    <Grid className='title1Bold text-text-primary'>
                        {titleText}
                    </Grid>
                    <Grid className='bodyRegular text-text-primary'>
                        {descriptionText}
                    </Grid>
                    <Grid>
                        <Button 
