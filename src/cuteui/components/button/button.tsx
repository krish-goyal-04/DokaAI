...
interface ButtonProps {
  ...
  color?: 'primary' | 'secondary' | 'success' | 'error' | 'warning' | 'info' | 'light' | 'dark' | undefined;
...
const buttonStyles = {
  ...
  disabled: {
    contained: 'dark:hover:bg-background-disabled/70 bg-background-disabled cursor-not-allowed text-action-disabled backdrop-blur-xl hover:bg-background-disabled/70',
    outlined: 'dark:hover:bg-text-hint/70 bg-text-hint cursor-not-allowed text-action-disabled backdrop-blur-xl hover:bg-text-hint/70 border-[1px] dark:border-action-disabled ',
    close: 'dark:hover:bg-text-hint/70 bg-text-hint cursor-not-allowed text-action-disabled backdrop-blur-xl hover:bg-text-hint/70 border-[1px] dark:border-action-disabled ',
  ...
  return (
    ...
    >
      <div className='bodyMedium flex items-center gap-2.5'>
        {icon && position === 'leading' && <Image src={variant === 'contained' ? WhiteCommonIcon : GreenCommonIcon} alt='icon' width={20} height={20} />}
        {text}
        {icon && position === 'trailing' && <Image src={variant === 'contained' ? WhiteCommonIcon : GreenCommonIcon} alt='icon' width={20} height={20} />}
      </div>
      {children}
    </button>
  );
